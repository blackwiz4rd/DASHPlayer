...
public abstract class DashAlgorithm extends Thread {
    ...
    //Constants declaration, this file is the mpd downloaded by the server
     ...
     *
     * @param player         player which has to playback the video, it also helps making stats
    ...
    /**
     * @brief Closes the MDP Session
     */
    abstract void closeMDPSession();

    /**
     * Method called to run the code in background.
     * All DASH algorithms work by taking note of the previous bitrate of the latest downloaded segment.
     * The MPD file is downloaded from the server, and information is parsed to know the available bitrates, complexities (SSIM) and qualities.
     * Maximum bitrate, segment duration and qualities are set according to the algorithm used.
     * Every DASH algorithm downloads the first segment for each available bandwith according to the MPD initialization parameter from tag <SegmentTemplate></SegmentTemplate>
     * After downloading the first segment the algorithm runs in a loop to download the next available segment.
     */
    @Override
    public void run() {
        // Downloading MPD file from server.
        try {
        ...
        System.out.println("Avaialble banwidths taken from MPD: " + s);
        ...
        for (int i = 1; i <= bitrates.length; i++) {
            try {
                //Downloads first segment that has RepresentationID: i for each bitrate. bigbuckbunny$i$/bigbuckbunny$i$_.mp4
